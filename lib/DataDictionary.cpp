...
namespace dicom
{
  namespace
  {
    ...
    std::pair<Tag, Item> MakeValueType(const DictionaryEntry& entry)
    {
    ...
    */
    struct DataDictionary : std::map<Tag, Item>
    {
      DataDictionary()
      {
        std::transform(DICT_ENTRIES, DICT_ENTRIES + sizeof(DICT_ENTRIES) / sizeof(DictionaryEntry),
          std::inserter(*this, begin()), MakeValueType);
      }
      ...
      {
        return boost::str(boost::format("(%1$04.0X,%2$04.0X)") %GroupTag(tag) %ElementTag(tag));
      }
